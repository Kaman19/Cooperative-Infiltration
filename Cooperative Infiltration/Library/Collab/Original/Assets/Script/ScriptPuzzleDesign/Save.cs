...
public class Save : MonoBehaviour
{
    ...
    public void Sauvegard()
    {
       ...
       for(int i=1;i<tab.Length;i++)
       {
        ...
        if(File.Exists(Application.dataPath + "/Save/Facile/P4/" + sauvSol))
        {
            Debug.Log("oucou");
            int nb = 1;
            string savSo = "Soluce";
            while(File.Exists(Application.dataPath + "/Save/Facile/P4/" + sauvSol))
            {
                nb++;
        ...
        File.WriteAllText(Application.dataPath + "/Save/Facile/P4/" + sauvSol, saveString);
